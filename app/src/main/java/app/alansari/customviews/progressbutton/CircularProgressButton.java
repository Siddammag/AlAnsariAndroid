...
public class CircularProgressButton extends Button {
    ...
    private void init(Context context, AttributeSet attributeSet) {
        mStrokeWidth = (int) getContext().getResources().getDimension(app.alansari.R.dimen.cpb_stroke_width);
        colorWhite = ContextCompat.getColor(context, app.alansari.R.color.colorWhite);
    ...
    private StrokeGradientDrawable createDrawable(int color) {
        GradientDrawable drawable = (GradientDrawable) getResources().getDrawable(app.alansari.R.drawable.cpb_background).mutate();
    ...
    private void initAttributes(Context context, AttributeSet attributeSet) {
        TypedArray attr = getTypedArray(context, attributeSet, app.alansari.R.styleable.CircularProgressButton);
        if (attr == null) {
            return;
        }
        ...
        try {
            ...
            mIdleText = attr.getString(app.alansari.R.styleable.CircularProgressButton_cpb_textIdle);
            mCompleteText = attr.getString(app.alansari.R.styleable.CircularProgressButton_cpb_textComplete);
            mErrorText = attr.getString(app.alansari.R.styleable.CircularProgressButton_cpb_textError);
            mProgressText = attr.getString(app.alansari.R.styleable.CircularProgressButton_cpb_textProgress);

            mIconComplete = attr.getResourceId(app.alansari.R.styleable.CircularProgressButton_cpb_iconComplete, 0);
            mIconError = attr.getResourceId(app.alansari.R.styleable.CircularProgressButton_cpb_iconError, 0);
            mCornerRadius = attr.getDimension(app.alansari.R.styleable.CircularProgressButton_cpb_cornerRadius, 0);
            mPaddingProgress = attr.getDimensionPixelSize(app.alansari.R.styleable.CircularProgressButton_cpb_paddingProgress, 0);

            int blue = getColor(app.alansari.R.color.cpb_blue);
            int white = getColor(app.alansari.R.color.cpb_white);
            int grey = getColor(app.alansari.R.color.cpb_grey);

            int idleStateSelector = attr.getResourceId(app.alansari.R.styleable.CircularProgressButton_cpb_selectorIdle,
                    app.alansari.R.color.cpb_idle_state_selector);
            mIdleColorState = getResources().getColorStateList(idleStateSelector);

            int completeStateSelector = attr.getResourceId(app.alansari.R.styleable.CircularProgressButton_cpb_selectorComplete,
                    app.alansari.R.color.cpb_complete_state_selector);
            mCompleteColorState = getResources().getColorStateList(completeStateSelector);

            int errorStateSelector = attr.getResourceId(app.alansari.R.styleable.CircularProgressButton_cpb_selectorError,
                    app.alansari.R.color.cpb_error_state_selector);
            mErrorColorState = getResources().getColorStateList(errorStateSelector);

            mColorProgress = attr.getColor(app.alansari.R.styleable.CircularProgressButton_cpb_colorProgress, white);
            mColorIndicator = attr.getColor(app.alansari.R.styleable.CircularProgressButton_cpb_colorIndicator, blue);
            mColorIndicatorBackground =
                    attr.getColor(app.alansari.R.styleable.CircularProgressButton_cpb_colorIndicatorBackground, grey);
        } finally {
            attr.recycle();
        }
    }
    ...
    private void morphCompleteToIdle() {
        ...
        animation.setToStrokeColor(colorWhite);
