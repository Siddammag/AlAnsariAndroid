...
public class TransactionTravelCompDetailsActivity extends AppCompatActivity implements View.OnClickListener, OnWebServiceResult, TextView.OnEditorActionListener, LogOutTimerUtil.LogOutListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_transaction_travel_comp_details);
        context = this;
            ...
            if (txnDetailsData != null) {
                ...
                if (sourceType.equalsIgnoreCase(Constants.TRANSACTION_TYPE_WU)) {
                    ...
                    if (Validation.isValidString(String.valueOf(txnDetailsData.getTxnCode()))) {
                        ...
                        if (String.valueOf(txnDetailsData.gettRANSACTIONDATAWC()) != null && String.valueOf(txnDetailsData.gettRANSACTIONDATAWC().getCreatedDate()).trim().length() > 0)
                            tvPaymentDate.setText(CommonUtils.getDateFromMilliSec(Long.valueOf(String.valueOf(txnDetailsData.gettRANSACTIONDATAWC().getCreatedDate()))));
...
//------------------------------------------------------DKG-----------------------------------------
            ...
            case UPDATE_TRAVEL_CARD_PAYMENT:
                ...
                if (status == 1) {
                    try {
                        if (response.getString(Constants.STATUS_MSG).equals(Constants.SUCCESS)) {
                            intent = new Intent(context, TransactionPageTravelCardComp.class);
                            intent.putExtra(Constants.OBJECT, txnDetailsData);
