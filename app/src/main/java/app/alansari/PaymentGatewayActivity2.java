...
public class PaymentGatewayActivity2 extends AppCompatActivity implements OnWebServiceResult, LogOutTimerUtil.LogOutListener {
    ...
    public void onBackPressed() {
        ...
        ((Button) exitDialog.findViewById(app.alansari.R.id.dialog_btn_yes)).setText(getString(app.alansari.R.string.btn_yes));
                ...
                new Handler().postDelayed(new Runnable() {
                    ...
                    public void run() {
                        JsonObjectRequest jsonObjReq = new CallAddr().executeApi(new APIRequestParams().setTxnCompletedRemittanceApi(
                                ...
                                Request.Method.PUT, PaymentGatewayActivity2.this);
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (getIntent().getExtras() != null) {
            ...
            /*if (getIntent().getExtras().getString(Constants.SOURCE).equalsIgnoreCase(Constants.TYPE_SEND_MONEY)) {
                ...
                if (Constants.DYNAMIC_URL.equalsIgnoreCase(Constants.BASE_URL_TESTING) || Constants.BASE_URL.equalsIgnoreCase(Constants.WU_BASE_URL_TESTING)) {
                   //Test
            ...
            if (getIntent().getExtras().getString(Constants.SOURCE).equalsIgnoreCase(Constants.TYPE_SEND_MONEY)) {
                ...
                if (Constants.TESTING_ENVIROMENT == 1) {
                    //Test
                    webView.loadUrl(Constants.PAYMENT_GATEWAY_BASE_URL_TEST1 + url);
                    Log.e("vgcfgcyf", "" + Constants.PAYMENT_GATEWAY_BASE_URL_TEST1 + url);
                } else {
                    //Live
                    webView.loadUrl(Constants.PAYMENT_GATEWAY_BASE_URL1 + url);
                }
    ...
    private void init() {
        ...
        webView = (WebView) findViewById(app.alansari.R.id.web_view);
                ...
                public void run() {
                    ...
                    if (getIntent().getExtras().getString(Constants.SOURCE_TYPE) != null && getIntent().getExtras().getString(Constants.SOURCE_TYPE).equalsIgnoreCase(Constants.TRANSACTION_TYPE_WU)) {
                        jsonObjReq = new CallAddr().executeApi(new APIRequestParams().setFundCompletedRemittanceApi(
                                CommonUtils.getUserId(), dataObject.getTxnCode(), getServiceType(dataObject.getServiceType()), LogoutCalling.getDeviceID(context), sessionTime), Constants.WU_SET_FUND_COMPLETED_REMITTANCE_API_URL,
                        ...
                        jsonObjReq = new CallAddr().executeApi(new APIRequestParams().setFundCompletedRemittanceApi(
                                CommonUtils.getUserId(), dataObject.getTxnCode(), getServiceType(dataObject.getServiceType()), LogoutCalling.getDeviceID(context), sessionTime), Constants.SET_FUND_COMPLETED_REMITTANCE_API_URL,
                                (status.equalsIgnoreCase(Constants.TRANSACTION_STATUS_PENDING)) ? SET_FUND_COMPLETED_REMITTANCE_API : SET_FUND_REJECTED_REMITTANCE_API,
                                Request.Method.PUT, PaymentGatewayActivity2.this);
                    }
...
//                    if (status.equalsIgnoreCase(Constants.TRANSACTION_STATUS_COMPLETED))
                    ...
                    try {
                        ...
                        if (response.getString(Constants.STATUS_MSG).equals(Constants.SUCCESS)) {
                            ...
                            if (getIntent().getExtras().getString(Constants.SOURCE) != null && getIntent().getExtras().getString(Constants.SOURCE).equalsIgnoreCase(Constants.SOURCE_PAYMENT_MODE)) {
                                Intent intent = new Intent(context, TransactionPageTravelCardComp.class);
    ...
    private class MyWebViewClient extends WebViewClient {
        ...
        public void onPageStarted(WebView view, String url, Bitmap favicon) {
            ...
            LogUtils.d("DKG ok", "URL " + url + " " + successUrl);
            if (getIntent().getExtras().getString(Constants.SOURCE).equalsIgnoreCase(Constants.TYPE_SEND_MONEY)) {
