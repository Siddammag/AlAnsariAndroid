 ...
 * AlAnsari
 */
public class TransactionDetailsActivity extends AppCompatActivity implements View.OnClickListener, OnWebServiceResult, TextView.OnEditorActionListener , LogOutTimerUtil.LogOutListener  {
    ...
    private TextView tvTotalReceive, tvTotalReceiveCurrencyCode, tvTotalToPay, tvTotalToPayCurrencyCode, tvLearnToTranferMessage;
            ...
            if (txnDetailsData != null) {
                ...
                if (sourceType.equalsIgnoreCase(Constants.TRANSACTION_TYPE_WU)) {
                   ...
                   /* tvPaymentMode.setText("Pay at Branch");
                    ...
                    }/*else if(txnDetailsData.getServiceType().equalsIgnoreCase(Constants.AREX_MAPPING) && txnDetailsData.getTxnType().equalsIgnoreCase("BT")){
                        txnDetailsData.setBeneficiaryData(txnDetailsData.getBeneficiaryDataBTCE());
                        txnDetailsData.setTransactionData(txnDetailsData.getTransactionDataBTCE());
                    }*/
//---------------------------------------------DKG--------------------------------------------------

                   /* else if(txnDetailsData.getTxnType().equalsIgnoreCase("CP")){
                        txnDetailsData.setBeneficiaryData(txnDetailsData.getBeneficiaryDataTCE());
                        txnDetailsData.setTransactionData(txnDetailsData.getTransactionDataCE());
                    }
                    else if(txnDetailsData.getTxnType().equalsIgnoreCase("CC")){
                        txnDetailsData.setBeneficiaryData(txnDetailsData.getBeneficiaryDataTCC());
                        txnDetailsData.setTransactionData(txnDetailsData.getTransactionDataCC());
                    }*/
//-----------------------------------------------------------------------------------------------
                    if (Validation.isValidString(txnDetailsData.getBeneficiaryData().getBeneficiaryName())) {
                    ...
                    } else if (Validation.isValidString(txnDetailsData.getBeneficiaryData().getCard_holder_name())) {
                        ...
                        // tvBranchName.setText(txnDetailsData.getTransactionData().getAgent_name());
                        ...
                        if(txnDetailsData.getTransactionData().getBenBankName()!=null){
                            tvBankName.setText(txnDetailsData.getTransactionData().getBenBankName());
                        }else{
                            tvBankName.setText(txnDetailsData.getTransactionData().getRoutine_bank_name());
                        }
                  ...
                  /*  Log.e("khfcuiwhfcu", "" + sourceType);
                    Log.e("khfcuiwhfcu", "" + txnDetailsData.getTxnRecType());
                    Log.e("khfcuiwhfcu", "" + txnDetailsData.getTxnType());*/
                    ...
                    } else if (Validation.isValidString(txnDetailsData.getBeneficiaryData().getCard_number())) {
                        tvAccountNumber.setText(Validation.getMaskedString(txnDetailsData.getBeneficiaryData().getCard_number()));
                        ((TextView) findViewById(app.alansari.R.id.account_number_label)).setText(getString(app.alansari.R.string.card_number));

                    } else if (Validation.isValidString(txnDetailsData.getBeneficiaryData().getBene_account_no())) {
                   ...
                   /* if (txnDetailsData.getBeneficiaryData().getPayMode().equalsIgnoreCase(Constants.PAYMENT_MODE_BRANCH_PAY)) {
                        ...
                        if (Validation.isValidRate(txnDetailsData.getTxnReferenceNum())) {
                            ...
                            etReferenceNumber.addTextChangedListener(new TextWatcher() {
                                ...
                                @Override
                                public void onTextChanged(CharSequence s, int start, int before, int count) {
                                    if (txnDetailsData.getBeneficiaryData().getPayMode().equalsIgnoreCase(Constants.PAYMENT_MODE_BANK_TRANSFER)) {
                                        if ((!TextUtils.isEmpty(etReferenceNumber.getText().toString().trim())) && etReferenceNumber.getText().toString().trim().length() > 0) {
                                            btnNext.setEnabled(true);
                                        } else {
                                            btnNext.setEnabled(false);
                  ...
                  tvTotalReceive.setText(CommonUtils.addCommaToString(txnDetailsData.getTransactionData().getTxnAmount()));
                    ...
                    tvTotalToPayCurrencyCode.setText("AED");
                    if (!txnDetailsData.getBeneficiaryData().getPayMode().equalsIgnoreCase(Constants.PAYMENT_MODE_PRIORITY_PAY)) {
                        clockLayout.setVisibility(View.VISIBLE);
                        if (txnDetailsData.getCreatedDate() != null && txnDetailsData.getTxnExpLimit() != null) {
                            try {
                                currentTimeMills = txnDetailsData.getCurrentTime() != null ? Long.valueOf(txnDetailsData.getCurrentTime()) : System.currentTimeMillis();
            ...
            case app.alansari.R.id.next_btn:
                if (sourceType.equalsIgnoreCase(Constants.TRANSACTION_TYPE_WU)) {
                    ...
                    if (txnDetailsData.getTxnPayType().equalsIgnoreCase(Constants.PAYMENT_MODE_BRANCH_PAY)) {
                        ...
                        if (etReferenceNumber.getText() != null && etReferenceNumber.getText().toString().trim().length() > 0) {
                            submitReferenceNumRemittanceApi();
                        } else {
                            Toast.makeText(context, "Please enter a valid reference number", Toast.LENGTH_SHORT).show();
                        }
                    } else if (txnDetailsData.getTxnPayType().equalsIgnoreCase(Constants.PAYMENT_MODE_PRIORITY_PAY)) {
